...
check_age_categories <- function (data) {
  ...
  for (k in 1:2) {
    ...
    age_category_match <- with(
      age_data,
      Age == age_category_from_extra_column(Age_as_indicated_by_Parks, 
                                            age_numeric[k])
    )
    age_mismatch_sample <- 
    ...
    if (!all(age_category_match)) {
      warning(
        paste0(
          "Age category does not match the value indicated by the park. Please check sample ", age_mismatch_sample
        )
      )
    }
  }
...
check_complete_entries <- function (data) {
  entries_incomplete <- data |> dplyr::select(
...
check_measured_chemicals_against_the_overview <- function (data, chemicals) {
  ...
  if (!all(measured_chemicals %in% chemicals)) {
    warning(
      "A chemical was measured that was not in the overview. Possible misspelling."
    )
  }
  if (!all(chemicals %in% measured_chemicals)) {
    warning(
      "A chemical that is in the overview was not measured anywhere. Shall we include it as a non-detect everywhere?"
    )
  }
}

...
sheets <- expand.grid(Park = names(data_positions), Data_version = c("winter", "summer"))
raw_data_list <- list()
...
raw_data <- bind_rows(raw_data_list) |> 
  mutate(  # Format the dates and relabel the factors
...
clean_data <- raw_data |> 
  filter(!(Sample_number %in% c("F19b", "F20b"))) |>  # filter out duplicate rows
  filter(Sample_number != "G21")  |> # Drop the only C. capreolus
  dplyr::select(  # Filter out  unnecessary information
    -c("Interne Nummer", "Bemerkung", "x", "y", "Revier Lang", "Gemeinde", 
       "Jagdbezirk", "Gewicht [kg]", "Anmerkung zum Gewicht")
    )
...
chem_categories <- read_excel(
  ...
  Chemical, primary_category, Detection_threshold
) %>% mutate(
  Detection_threshold = as.numeric(Detection_threshold),
  primary_category = factor(
    primary_category, 
    labels = c("Anthropogenic pollution", "API", "Industrial chemical", "PAH", 
               "PCP", "Pesticide", "POP", "Plasticizer")
  ),
  ...
  Chemical = case_when(
    ...
    Chemical == "2,4,4'-Trichlorobiphenyl (PCB #52)" ~ "2,2',5,5'-Tetrachlorobiphenyl (PCB #52)",
    .default = Chemical
  )
)
...
check_measured_chemicals_against_the_overview(clean_data, chem_categories$Chemical)

...
### Species
The species explanatory variable is categorical with 2 groups:
- C. elaphus,
- D. dama.

In some parks only samples from C. elaphus were collected. For this reason, we carry out the analysis only for parks, where samples from both species were collected - Hainich, Jasmund and Vorpommerische Boddenlandschaft. As a result, the reference category for the park variable is now Hainich instead of Bayerischer Wald.
